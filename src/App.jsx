...
const SITE_KEYS = {
  google: "",AIzaSyDiASNmASU6JBNUfhswoySYFgCwfxn9fFI"  → 🌍 Photoreal works for all visitors
  mapbox: "", pk.eyJ1IjoicG9zZXk2MjgiLCJhIjoiY21yODBycmYzMWhwbDJ4cHc1M3NsdzcyNCJ9.UiRU2R-Kky2X5joV54Jrhg"   → HD satellite for all visitors
};
        ...
        return (
          <g key={it.uid} onPointerDown={(e) => down(e, it)} style={{ cursor: "grab" }}>
            <rect x={x} y={y} width={Math.max(w, 6)} height={Math.max(d, 6)}
              fill={green ? `url(#leaf${sfx})` : `url(#hatch${sfx})`}
...
async function fetchTerrain(lat, lon, acres) {
  ...
  const url = "https://api.open-elevation.com/api/v1/lookup?locations=" + pts.map((p) => `${p[0].toFixed(5)},${p[1].toFixed(5)}`).join("|");
...
Features:\n${spec || "Vacant land only."}`);
  ...
  const demolish = (uid) => { snap(); removeItem(uid, true); addItem(DEMO_ITEM, 0, {}, true); };
  ...
  if (!location) {
    return (
      <div className="app"><Style />
        <header className="hdr">
          <div className="hdr-mark">⌂</div>
          <div><h1>TapaCasa</h1><p className="hdr-sub">SHEET 1 — SITE SELECTION · ANYWHERE ON EARTH · v11-LAUNCH</p></div>
        </header>
        <div className="loc-wrap">
