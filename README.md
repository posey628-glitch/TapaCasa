...
## v12-INTEL — server env vars (Netlify: Site configuration → Environment variables)

- `ANTHROPIC_API_KEY` — AI features for all visitors (required for AI).
- `GOOGLE_API_KEY` — powers Google Elevation (better slope data), Geocoding
  (better address search), and the "📍 What's nearby" panel. IMPORTANT: make a
  SECOND Google key for this one with NO website restriction (server calls have
  no website); restrict it by API instead to: Elevation API, Geocoding API,
  Places API. Enable those three in APIs & Services → Library. Your first
  (website-restricted) key stays in SITE_KEYS for Photoreal + Street View.
- `REGRID_TOKEN` — optional, paid (regrid.com/api): "▦ Load official parcel"
  pulls exact legal boundaries for every visitor, token hidden server-side.
- Also enable "Street View Static API" on your FIRST (website) key to get
  street-level photos of parcels in the app.

After adding/changing env vars: Deploys → Trigger deploy.
